...
def prompt2motion():
    openai.api_key = "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"
    messages = [
        ...
        {"role": "user", "content": "如果是调整追踪参数的指令，回答请以如下的形式输出：\n"
                                    ...
                                    "如果是运动控制类的指令，程序回答请直接输出在程序代码块```python ```里。不要导入任何库。不要导入任何库。不要导入任何库。"
        ...
        #                             "记住：只要输出程序代码块，然后在代码块之外输出语句'动作执行完毕！'。只要输出程序代码块，然后在代码块之外输出语句'动作执行完毕！'。"},
                                    ...
                                    "记住：一定要输出程序代码块里，一定要输出程序代码块里，一定要输出程序代码块里。"},
    ...
    try:
        while True:
            ...
            print(ans.content)
            code, instructions, text = text_process(ans.content)
            print(ans.role + ": " + text + '\n')
            if code == "":
                continue
            write_code(code)
